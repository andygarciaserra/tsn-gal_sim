...
for j in range(len(method)):
    ...
    for i in range(len(sys)):
        ...
        t = np.append(t, (tf-ti))
        nt = np.append(nt, n[i])
    np.savetxt(OUTDIR+'timevsN_'+method[j]+'_noint.txt',np.transpose([t,nt]),header='Time(s)\tNparts')

    #Plotting computing time vs Npart (no interaction between stars):
data = np.array([[],[]])
plt.figure(dpi=150)
for j in range(len(method)):
    data = np.loadtxt(OUTDIR+'timevsN_'+method[j]+'_noint.txt', skiprows=1)
...
plt.ylabel('t (s)')
plt.legend()
plt.savefig(FIGDIR+'timevsN_noint.png')
